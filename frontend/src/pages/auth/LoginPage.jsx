...
import { useSignIn, useUser as useClerkUser, useClerk } from '@clerk/clerk-react';
...
export default function LoginPage() {
  ...
  // After Clerk Google sign-in completes (redirect back), sync with Laravel backend
  useEffect(() => {
    if (isSignedIn && clerkUser && !syncingRef.current) {
      ...
      if (email) {
        googleLogin(email, name)
          .then(() => {
            // Sign out of Clerk (we only use it for OAuth, auth is managed by Laravel)
            signOut();
            navigate('/dashboard');
          })
          .catch((err) => {
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-gray-100">
      ...
      <div className="flex items-center justify-center px-4 mt-8">
        <div className="w-full max-w-md bg-white rounded-2xl shadow-xl shadow-gray-200/50 p-8 border border-gray-100">
          <h1 className="text-2xl font-bold text-center text-gray-800 tracking-tight">Welcome back</h1>
          <p className="text-center text-[13px] text-gray-500 mt-1 mb-6">Please enter your details to sign in.</p>
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-2 py-2.5 border border-gray-200 rounded-xl hover:bg-gray-50 hover:border-gray-300 transition-all duration-200 cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {googleLoading ? (
          ...
          <p className="mt-6 text-center text-[13px] text-gray-500">
            ...
            <Link to="/register" className="text-blue-600 font-semibold hover:underline">Sign up for free</Link>
          </p>
        </div>
      </div>
    </div>
